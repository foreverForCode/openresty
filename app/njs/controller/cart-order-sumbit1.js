...
var cartIdsStr = sessionStorage.cartIdsStr;
...
var addressId = 0;
//清关费5元 20160114 add cjw
var clearFee = countPrice >= 499 ? 0 : 5;
//邮费：购买总价大于500的为40元，小于500的为20元 20160114 add cjw
var postFee = countPrice >= 499 ? 0 : 20;
var freeCount = 499;
...
$(function () {
    ...
    $.ajax({
        type: "post",
        data: {"cartIds": cartIds, "discountAmt": discountAmt,"isSingle":isSingle},
            ...
            if (errCode == 10000) {
            	if(cartIdsStr!=''){
            		if (couponData.data.length != 0) {
                        ...
                        findTmn(params.tmn, countPrice - discountAmt - rebatePrice);
                        coups.attr("price", discountAmt);
                        coups.find(".amount").text(discountAmt);
                    } else {
                        findTmn(params.tmn, countPrice - rebatePrice);
                    }
            	}else{
                    findTmn(params.tmn, countPrice - rebatePrice);
            	}
...
function defaultCoupon() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.total > 0) {
                discountAmt = data.data[0].couponMinus;
...
function findTmn(tmn, countPrice) {
	if(discountAmt == 0){//未使用哟挥拳
		countPriceOnion = sessionStorage.countPriceOnion;
	} else {
		countPriceOnion = sessionStorage.countPriceOnion - discountAmt;
	}
	...
	} else if(newStore == "true" && countPriceOnion >= 299){//新店299免邮
		$("#coumosfix").text("0.0");
        $("#shipfix").text("0.0");
        $("#totalCount").text(jems.formatNum(countPrice, 2));
        $("#tipText").text("新开商铺前七天单笔达299元免邮费");
        $(".newStore").show();
        $("#freeshipp").hide();
	} else{
		$.ajax({
	        type: "post",
	        data: {"tmn": tmn, "countPrice": countPriceOnion},
	        ...
	        success: function (data) {
	            if (data.success) {
	                ...
	                $("#coumosfix").text("5.0");
	                $("#shipfix").text("20.0");
	                $("#totalCount").text(jems.formatNum(countPrice + 25, 2));
	                $("#cost_ywbtn").show();
	                $("#freeshipp").show();
