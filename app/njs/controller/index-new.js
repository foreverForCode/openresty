...
$(function(){
    ...
    /**
     * 功能：下拉一定的距离，搜索栏隐藏，导航栏固定在顶部
     * 
     * */
    var searchHeight = $("#header").css('height').replace("px","");

    searchHeight = Number(searchHeight);

    window.addEventListener('scroll', function(){
        var walker = window.scrollY;
        console.log(walker);

        if(walker>searchHeight){
            $("#header").hide();
            $(".mstopnav ").addClass('msheader');
            $(".mstopnav ").css('background-color','#fff');
        }else{
            $("#header").show();
            $(".mstopnav ").removeClass('msheader');
        }
    }, false);
