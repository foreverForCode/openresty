    ...
    function allCartCount() {
        ...
        function getCartCount() {
            ...
            if (priceTotal == 0 || priceTotal == 0.00 ){
                $("#other_tip").hide();
                $("#newstore").show();
                $("#newstore").text(newStore?"新店铺满299元免邮":"全场满499元免邮 ");
            } else if(newStore && priceTotal > 0 && priceTotal < 299){//新开店铺七天299包邮
                $("#newstore").hide();
                $("#other_tip").text("运费25元，还差"+jems.formatNum(299-priceTotal)+"元即可免邮，去凑单");
                $("#other_tip").show();
            } else if(newStore && priceTotal >= 299 &&  priceTotal < 1000){//新开店铺七天299包邮
                $("#newstore").hide();
                $("#other_tip").text("已免邮,再逛逛");
                $("#other_tip").show();
            } else if(priceTotal < 499){
                $("#newstore").hide();
                $("#other_tip").text("运费25元，还差"+jems.formatNum(499-priceTotal)+"元即可免邮，去凑单");
                $("#other_tip").show();
            } else if(priceTotal >= 499 && priceTotal < 1000){
                $("#newstore").hide();
                $("#other_tip").text("已免邮,再逛逛");
                $("#other_tip").show();
            } else {
