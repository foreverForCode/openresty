...
cartIds = sessionStorage.cartIds;//params.cartIds;
...
$(function(){
	...
	$.ajax({
		type : "post",
		data : {"cartIds":cartIds,"discountAmt":discountAmt},
...
var joinOrder = function(){
	jems.goUrl("../search-list.html?maxamt=" + (1000 - countPrice));
}
...
function submitOrder(){
	...
	function createSod(){
		$.ajax({
			type: "post",
			data: {"cartIds": cartIds, "tmnId": tmnId, "addressId": addressId,"logistics":logisval, "sodUserId": "","couponNo":couponNo},
...
function getCartByCardIdsV1(amount,state){
	$.ajax({
		type : "post",
		data : {"cartIds":cartIds,"discountAmt":amount},
		url : msonionUrl+"cart/getCartByCardIds/v1",
		dataType: "json",
		success:function(result){
				result.data.url = url;
