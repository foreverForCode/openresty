...
$(function(){
	...
	if (tmn == 1 || tmn == 107) {
        ...
        return;
		// mBox.open({
		// 	content: "洋葱海外仓为邀请制销售，请联系您身边的店主/代理/洋葱客服",
		// 	btnName: ['寻找身边店主'],
...
function register(validate){
	...
	if (tmn == 1 || tmn == 107) {
		mBox.open({
			content: "洋葱海外仓为邀请制销售，请联系您身边的洋葱经纪人/代理/洋葱客服",
			btnName: ['确定'],
			btnStyle:["color: #0e90d2;"],
			maskClose:false,
			yesfun : function(){
	...
	$.ajax({
		type: 'POST',
		data:{"memberPhone":memberPhone,"yzm":authCode,"memberPassword":password1,"memberTmnId":tmn,"memberYCID":memberYCID,"memberFrom":0},
