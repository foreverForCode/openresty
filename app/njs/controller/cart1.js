...
var countPrice;
var countFreePrice;
var onionSumAmt = 0, otherSumAmt = 0, foreignSumAmt = 0, drinkSumAmt = 0;//海外仓总计价格
var messageonion = '';//库存不足弹出提示
var messagedrink = '';//库存不足弹出提示
var messageforeign = '';//库存不足弹出提示
...
$(function () {
    ...
    $("#allSelect").on("change", function (e) {
        ...
        seasAllSelect(seasobj, 0);
        foreignAllSelect(foreignobj, 0);
        drinkAllSelect(drinkobje, 0);
        if (messageonion != "") {
            seasobj.prop("checked", false);
        }
        if (messagedrink != "") {
        ...
        if (messageonion != "" || messagedrink != "" || messageforeign != "") {
            limitMsg("温馨提示:", messageonion + messagedrink + messageforeign + "</br>", null);
        }
    })
...
function settlement(){
    ...
    var cartIds = {foreignList: foreign, onionList: onion, wineFirstList: wineFirst};
 ...
 * 海外仓全选
 */
function seasAllSelect(obj, type) {
    ...
    if (that.prop('checked')) {
        $("#seasblock li").each(function () {
            ...
            if (!photoobj.hasClass("soldout1") && !photoobj.hasClass("soldout5")) {
            ...
            } else if(photoobj.hasClass("soldout1")) {
 ...
 * 酒水全选
 */
function drinkAllSelect(obj, type) {
    price = 0;
    var that = obj;
    messagedrink = '';
    if (that.prop('checked')) {
        $("#drinksblock li").each(function () {
            ...
            if (!photoobj.hasClass("soldout1") && !photoobj.hasClass("soldout5")) {
            	...
            	messagedrink += $(this).find("h3").html() + "库存不足</br>";
            }else{
            	messagedrink += $(this).find("h3").html() + "预热商品暂时不可下单</br>";
            }
        })
        if (messagedrink != '' && type == 1) {
            limitMsg("温馨提示:", messagedrink + "</br>", that);
        }
 ...
 * 贩外全选
 */
function foreignAllSelect(obj, type) {
    price = 0;
    var that = obj;
    messageforeign = '';
    if (that.prop('checked')) {
        $("#foreignblock li").each(function () {
            ...
            if (!photoobj.hasClass("soldout1") && !photoobj.hasClass("soldout5")) {
            	...
            	messageforeign += $(this).find("h3").html() + "库存不足</br>";
            }else{
            	messageforeign += $(this).find("h3").html() + "预热商品暂时不可下单</br>";
            }
        })
        if (messageforeign != '' && type == 1) {
            limitMsg("温馨提示:", messageforeign + "</br>", that);
        }
            ...
            if (data.errCode == 4001 || data.errCode == 4002) {	// 如果未登录，则跳至登录页面
                ...
                if (data.data == "" || data.data == null) {
                    ...
                    foreignList.forEach(function (v) {
                        ...
                        } else if (product.leSpecs[0].saleState == 5 || product.leSpecs[0].saleState == 10) {
                            foreignhtml += '<div class="photo jepor jecell-all ml10 soldout5">';
                        } else {
                            foreignhtml += '<div class="photo jepor jecell-all ml10">';
                        }
                        ...
                        foreignhtml += '<p class="jeell pr25 ">颜色：' + product.leSpecs[0].spec1.name + '色，尺码：' + product.leSpecs[0].spec2.name + '</p>';
                        foreignhtml += '</div>';
                    ...
                    seasList.forEach(function (v) {
                        ...
                        } else if (product.saleState == 5 || product.saleState == 10) {
                            seaslisthtml += '<div class="photo jepor jecell-all ml10 soldout5">';
                        } else {
                            seaslisthtml += '<div class="photo jepor jecell-all ml10">';
                        }
                    ...
                    aniList.forEach(function (v) {
                        ...
                        if (product.qty <= 0 || product.leStat == 2 || product.qty < v.num) {
                            ...
                            anihtml += '<div class="photo jepor jecell-all ml10 soldout5">';
                        } else {
                            anihtml += '<div class="photo jepor jecell-all ml10">';
                        }
                    ...
                    drinksList.forEach(function (v) {
                        ...
                        if (product.qty <= 0 || product.leStat == 2 || product.qty < v.num) {
                            ...
                            drinkshtml += '<div class="photo jepor jecell-all ml10 soldout5">';
                        } else {
                            drinkshtml += '<div class="photo jepor jecell-all ml10">';
                        }
...
function checkOnionMoney(dutyTotal){
    if(dutyTotal > 1000){
    	jems.mboxMsg("洋葱海外仓的商品一单不可超1000元");
    	return;
    }
}
...
function sumTotalMoney(countFreePrice, type) {
    ...
    var totalAmt = drinkSumAmt + foreignSumAmt + onionSumAmt;
